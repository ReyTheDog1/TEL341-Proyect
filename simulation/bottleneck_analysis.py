...
UMBRALES_CUELLO_BOTELLA = 50  # Umbral en Mbps para identificar un cuello de botella
...
class PuntoDeAcceso:
    ...
    def detectar_cuello_de_botella(self):
        return [tiempo for tiempo, carga in self.historial_carga if carga > UMBRALES_CUELLO_BOTELLA]
...
def run_bottleneck_simulation():
    ...
    cuellos_antes = {pa.id: pa.detectar_cuello_de_botella() for pa in puntos_de_acceso}
    ...
    env.run(until=SIM_TIME)
    cuellos_despues = {pa.id: pa.detectar_cuello_de_botella() for pa in puntos_de_acceso}

    return cuellos_antes, cuellos_despues
